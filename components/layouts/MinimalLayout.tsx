...
export function MinimalLayout({ title, mutedTextClass, heroCtaHref, ctaLabel, sections, navLinks = [], anchors = {}, surfaceClass, typographyScaleClass }: LayoutRenderProps) {
  return (
    <div className="mx-auto w-full max-w-5xl space-y-6">
      <header className={`cs-nav sticky top-2 z-20 border border-[var(--brand-border)] bg-[var(--brand-surface)]/90 px-4 py-3 backdrop-blur ${surfaceClass ?? ''}`}>
        <div className={`flex items-center justify-between ${typographyScaleClass ?? ''}`}>
          ...
          {navLinks.length > 0 && (
            <nav className={`cs-nav hidden items-center gap-4 text-xs ${mutedTextClass} md:flex`}>
      ...
      {sections.hero && <section className={`cs-section bg-[var(--brand-surface)] px-6 py-8 ${surfaceClass ?? 'shadow-sm'}`}>{sections.hero}</section>}
