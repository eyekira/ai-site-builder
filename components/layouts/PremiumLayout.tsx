...
export function PremiumLayout({ title, mutedTextClass, heroCtaHref, ctaLabel, sections, navLinks = [], anchors = {}, surfaceClass, typographyScaleClass }: LayoutRenderProps) {
  return (
    <div className="mx-auto w-full max-w-4xl space-y-10">
      <header className={`border border-[var(--brand-border)] bg-[var(--brand-surface)] px-6 py-5 backdrop-blur ${surfaceClass ?? ''}`}>
        <div className={`flex items-center justify-between gap-4 ${typographyScaleClass ?? ''}`}>
          ...
          <a href={heroCtaHref} className="bp-primary-cta rounded-md px-4 py-2 text-xs font-semibold">{ctaLabel}</a>
        </div>
      </header>

      {sections.hero && <section className={`border border-[var(--brand-border)] bg-[var(--brand-surface)] px-8 py-12 ${surfaceClass ?? ''}`}>{sections.hero}</section>}
      ...
      {(sections.reviews || sections.policies || sections.contact) && (
        <section className={`grid gap-8 ${sections.reviews && (sections.policies || sections.contact) ? 'md:grid-cols-2' : 'grid-cols-1'}`}>
          ...
          {(sections.policies || sections.contact) && (
            <article className={`border border-[var(--brand-border)] bg-[var(--brand-surface)] px-6 py-6 ${surfaceClass ?? ''}`}>{sections.policies ?? sections.contact}</article>
          )}
        </section>
      )}
