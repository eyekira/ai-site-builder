...
export function BakeryLayout({ title, mutedTextClass, heroCtaHref, ctaLabel, sections, navLinks = [], anchors = {}, surfaceClass, typographyScaleClass }: LayoutRenderProps) {
  return (
    <div className="mx-auto w-full max-w-5xl space-y-7">
      <header className={`border border-[var(--brand-border)] bg-[var(--brand-surface)] px-5 py-4 shadow-sm ${surfaceClass ?? ''}`}>
      ...
      {sections.hero && <section className={`border border-[var(--brand-border)] bg-[var(--brand-surface)] px-6 py-8 shadow-sm ${surfaceClass ?? ''}`}>{sections.hero}</section>}
